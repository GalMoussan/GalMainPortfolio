        ...
        <div role="tablist" aria-label="Job tabs" className="relative w-max z-10 flex flex-col">
          {jobs.map((job, i) => (
            <button
              ...
              className={`flex items-center w-[var(--tab-width)] h-[var(--tab-height)] px-5 py-0 border-l-2 bg-transparent font-mono text-[var(--fz-xs)] text-left whitespace-nowrap cursor-pointer transition-all duration-250 ease-custom ${
                activeTab === i
        ...
        <div className="relative ml-5 w-full">
          {jobs.map((job, i) => (
            <div
              ...
              className="w-full"
            >
              <h3 className="text-[var(--fz-xxl)] font-medium text-[var(--lightest-slate)] mb-[2px]">
                {job.title}{' '}
              ...
              <p className="font-mono text-[var(--fz-xs)] text-[var(--light-slate)] mb-[25px]">
                {job.range}
              </p>
              <ul className="list-none p-0">
                {job.duties.map((duty, j) => (
                  <li
                    key={j}
                    className="relative pl-[30px] mb-[10px] text-[var(--fz-lg)] text-[var(--slate)] before:content-['▹'] before:absolute before:left-0 before:text-[var(--green)] before:text-[var(--fz-sm)] before:leading-[26px]"
                  >
                    {duty}
                  </li>
                ))}
